...
void init_data(world_t * world){ 
	world-> background = load_image( "ressources/labyrinthe.bmp" );
		...
		if(world->deplace == 1){//le bas
			if(world->M[i].x <= world->x + 21  &&  world->M[i].x >= world->x + 12 ){
				for(int j = world->M[i].y; j < (world->M[i].y + world->M[i].haut); j++){
					if(j <= (world-> y + 20 + 10) && j >= world->y + 20 + 10){
						return 1;
					}
				}
			}
		
			if(world->M[i].y + world->M[i].haut <= world->y + 23 + 10  &&  world->M[i].y >= world->y + 10){
...
void handle_events(SDL_Event *event,world_t *world, SDL_Surface* screen){
	...
	//Si l'utilisateur a cliqué sur le X de la fenêtre
		...
		if( keystates[ SDLK_e ]){
			if(collision_coffre_Jean(world) == 1){
