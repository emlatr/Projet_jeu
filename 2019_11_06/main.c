...
int collision_coffre(world_t* world, coffre_t* c){
	if( (world->x + 21 >= c->cx ) && (world->x + 12 <= c->cx + SIZE_RIGHT_COFFRE ) ){
		if( (world->y + 26 >= c->cy ) && (world->y <= c->cy + 26 ) ){
			return 1;
		}
...
int collision_coffre_Jean(world_t *world){
	for(int i = 0; i<NB_COFFRE; i++){
		if (collision_coffre(world, &world->Co[i]) == 1){
			return 1;
		}
	}
	return 0;
...
void refresh_graphics(SDL_Surface *screen, world_t *world){
	...
	apply_surface(world->Lampe, screen, world->x-1480, world->y - 1500);
